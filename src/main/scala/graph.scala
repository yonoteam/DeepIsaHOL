...
This file is adapted from Makarius Wenzel's Isabelle/src/Pure/General/graph.scala
at https://isabelle.in.tum.de/
 */
...
class Graph[Key, A] private(
  ...
  def transitive_reduction_acyclic: Graph[Key, A] = {
    ...
    if (trans.iterator.exists({ case (x, (_, (_, succs))) => succs.contains(x) }))
      throw new Graph.User_Error("Cyclic graph")
